...
def test_filewise_table(num_files, values):
...
def test_segmented_table(num_files, num_segments_per_file, values):
...
def test_table_add():
    ...
    for num_files_1, num_files_2 in (
        ...
        np.testing.assert_equal(db['table']['scheme1'].get().dropna().unique(),
                                db['table1']['scheme1'].get().values)
        pd.testing.assert_series_equal(db['table']['scheme2'].get().dropna(),
    ...
    for num_files_1, num_files_2 in (
        ...
        db['table'] = db['table1'] + db['table2']
        assert db['table'].type == audformat.define.IndexType.SEGMENTED
        np.testing.assert_equal(db['table']['scheme2'].get().dropna().unique(),
                                db['table2']['scheme2'].get().values)
        pd.testing.assert_series_equal(db['table']['scheme1'].get().dropna(),
                                       db['table1']['scheme1'].get().dropna())


def test_table_extend():
    ...
    np.testing.assert_equal(db['table']['columns'].get().values,
                            np.array(['a', 'a']))
    index = pd.Index(['1.wav', '3.wav'], name=audformat.define.IndexField.FILE)
