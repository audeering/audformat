...
def add_table(
    ...
    if table_type == define.IndexType.FILEWISE:
        ...
        db[table_id] = Table(
            create_index(new_files, starts=starts, ends=ends),
            split_id=split_id,
            media_id=media_id,
        )
...
def create_db(minimal: bool = False) -> Database:
    ...
    db.media['microphone'] = AudioInfo(format='wav', sampling_rate=16000,
                                       channels=1)
    db.media['webcam'] = VideoInfo(format='avi', frames_per_second=25,
                                   resolution=[800, 600], depth=8, channels=3)
